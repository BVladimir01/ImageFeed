...
final class ImagesListViewController: UIViewController {
    ...
    private func configCell(_ cell: ImagesListCell, at indexPath: IndexPath) {
        guard let image = UIImage(named: imagesNames[indexPath.row]) else {
            assertionFailure("ImagesListViewController: Failed to create image for cell")
            return
        }
...
extension ImagesListViewController: UITableViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == showSingleImageSegueIdentifier {
            guard let viewController = segue.destination as? SingleImageViewController, let indexPath = sender as? IndexPath else {
                assertionFailure("ImagesListViewController: Failed to create ViewController or extract indexPath")
                return
            }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        guard let image = UIImage(named: imagesNames[indexPath.row]) else {
            assertionFailure("ImagesListViewController: Failed to extract image for calculating cell's height")
...
extension ImagesListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: ImagesListCell.reuseIdentifier, for: indexPath) as? ImagesListCell else {
            assertionFailure("ImagesListViewController: Failed to dequeue ImagesListCell")
